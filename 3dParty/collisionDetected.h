...
struct collisionDetectedPosition
{
	double x;
	double y;
	double z;
...
{
public:
	collisionDetected(std::vector<collisionDetectedPosition> vehcilePosition);
	~collisionDetected();
	...
	bool   IsTwoCoordinateConsistent(std::vector<collisionDetectedPosition> m_currentPositon, std::string m_direction);
...
private:
	...
	double InvPow(double n, int m);

	double coefficientMatrix[10][10]; //求多项式系数时需要的矩阵
	double m_fResult[3];

	std::vector<collisionDetectedPosition> m_vehiclePosition;
