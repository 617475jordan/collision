...
bool MSNet::changeVehicleSpeed(std::string &firstVehicleName, std::string &secondVehcileName)
{
	...
	for (unsigned int num = 0; num < m_setPreVehiclePara.size(); num++)
	{
		if (m_setPreVehiclePara[num].vehicleName == firstVehicleName)
		{
			m_firstVehilceFlag = true;
			continue;
		}
		else if (m_setPreVehiclePara[num].vehicleName == secondVehcileName)
		{
	...
	if (m_firstVehilceFlag == false)
	{
		...
		predVehilcePara m_vehiclePara;
		m_vehiclePara.vehicleName = firstVehicleName;

		m_vehiclePara.vehicleProtectedSpeed = libsumo::Vehicle::getMaxSpeed(firstVehicleName);
		m_setPreVehiclePara.push_back(m_vehiclePara);
	...
	if (m_secondVehilceFlag == false)
	{
		predVehilcePara m_vehiclePara;
		m_vehiclePara.vehicleName = secondVehcileName;
		m_vehiclePara.vehicleProtectedSpeed = libsumo::Vehicle::getMaxSpeed(secondVehcileName);
		m_setPreVehiclePara.push_back(m_vehiclePara);
	}
...
bool MSNet::PredictCollision(predVehilcePara &m_predFisrtVehilcePara, predVehilcePara &m_predSecondVehilcePara, int &PositionNum, unsigned char &LevelCoefficient)
{
	if (PositionNum <= 0|| LevelCoefficient<=0)
	{
		return false;
	}
	int m_iFinalPointPosition = PositionNum - 1;
	double m_fMaxVehicleDistance = (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].length > m_predSecondVehilcePara.m_collisionDetectedPosition[0].length)
		? m_predFisrtVehilcePara.m_collisionDetectedPosition[0].length : m_predSecondVehilcePara.m_collisionDetectedPosition[0].length;
	m_fMaxVehicleDistance = m_fMaxVehicleDistance * 2;

	//bool m_bFlag = false;
	if (m_predFisrtVehilcePara.m_bTraceIsJudgePositionYChange == false && m_predSecondVehilcePara.m_bTraceIsJudgePositionYChange == false
		&& abs(m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x - m_predSecondVehilcePara.m_collisionDetectedPosition[0].x) <= m_fMaxVehicleDistance)//??????§Ò????????????????????????
	{
		double m_initialPositionY = m_predFisrtVehilcePara.m_collisionDetectedPosition[0].y - m_predSecondVehilcePara.m_collisionDetectedPosition[0].y;
		double m_finalPositionY = m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].y -
			m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].y;
		...
		if (m_initialPositionY*m_finalPositionY <= 0)
		{
			...
			//changeVehicleSpeed(m_predFisrtVehilcePara.vehicleName, m_predSecondVehilcePara.vehicleName);
	...
	if (m_predFisrtVehilcePara.m_bTraceIsJudgePositionXChange == false && m_predSecondVehilcePara.m_bTraceIsJudgePositionXChange == false
		&& abs(m_predFisrtVehilcePara.m_collisionDetectedPosition[0].y - m_predSecondVehilcePara.m_collisionDetectedPosition[0].y) <= m_fMaxVehicleDistance)//??????§Ò????????????????????????
	{
		double m_initialPositionX = m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x - m_predSecondVehilcePara.m_collisionDetectedPosition[0].x;
		double m_finalPositionX = m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x
			- m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x;
	...
	if (m_predFisrtVehilcePara.m_bTraceIsJudgePositionYChange == false && m_predSecondVehilcePara.m_bTraceIsJudgePositionYChange == true)//consdier that position which has been sorted,where the min position may be the first position
	{
		if (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x >= m_predSecondVehilcePara.m_collisionDetectedPosition[0].x
			&&m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x <= m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x
			/*&&abs(m_predFisrtVehilcePara.m_collisionDetectedPosition[0].y-m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x)<=(m_predFisrtVehilcePara.m_collisionDetectedPosition[0].width+m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].width)/2.0*/)
		{
			//m_mapTwoVehilceNameCollision.insert(pair<std::string, std::string>(vehicleName[i], vehicleName[j]));
			/*changeVehicleSpeed(m_predFisrtVehilcePara.vehicleName, m_predSecondVehilcePara.vehicleName);
			m_bFlag = true;*/
		...
		{
			std::vector<predVehilcePara>::iterator m_vecVehiclePar;
			for (m_vecVehiclePar = m_setPreVehiclePara.begin(); m_vecVehiclePar != m_setPreVehiclePara.end(); m_vecVehiclePar++)
			{
				if (m_vecVehiclePar->vehicleName == m_predSecondVehilcePara.vehicleName || m_vecVehiclePar->vehicleName == m_predFisrtVehilcePara.vehicleName)
				{
					libsumo::Vehicle::setMaxSpeed(m_vecVehiclePar->vehicleName, m_vecVehiclePar->vehicleProtectedSpeed);
					//libsumo::Vehicle::getAccel()
				}
			}
		}
	}
	else if (m_predFisrtVehilcePara.m_bTraceIsJudgePositionYChange == true && m_predSecondVehilcePara.m_bTraceIsJudgePositionYChange == false)
	{
		if (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x < m_predSecondVehilcePara.m_collisionDetectedPosition[0].x
			&&m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x > m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x
			/*&&abs(m_predFisrtVehilcePara.m_collisionDetectedPosition[0].y - m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x) <= (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].width + m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].width) / 2.0*/)
	...
	else if (m_predFisrtVehilcePara.m_bTraceIsJudgePositionXChange == true && m_predFisrtVehilcePara.m_bTraceIsJudgePositionYChange == true &&
		m_predSecondVehilcePara.m_bTraceIsJudgePositionXChange == true && m_predSecondVehilcePara.m_bTraceIsJudgePositionYChange == true)
	{
		std::vector<collisionDetectedPosition> m_cmpVehiclePosition(PositionNum);
		...
		for (unsigned int num = 0; num < PositionNum; num++)
		{
			m_fPolynomialCoefficient[num] = m_predFisrtVehilcePara.m_dVehiclePolynomialCoefficient[num] - m_predSecondVehilcePara.m_dVehiclePolynomialCoefficient[num];
		}
		...
		if (m_bMeetPointFlag == true && m_iEquationSolutionNum > 0)
		{
			...
			for (unsigned char m_cEquationSolutionNum = 0; m_cEquationSolutionNum < (unsigned char)m_iEquationSolutionNum; m_cEquationSolutionNum++)
			{
				...
				double m_fFirstVehicleMinPosition = (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x < m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x) ?
					m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x : m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x;
				double m_fFirstVehicleMaxPosition = (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x > m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x) ?
					m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x : m_predFisrtVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x;
				double m_fSecondVehicleMinPosition = (m_predSecondVehilcePara.m_collisionDetectedPosition[0].x < m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x) ?
					m_predSecondVehilcePara.m_collisionDetectedPosition[0].x : m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x;
				double m_fSecondVehicleMaxPosition = (m_predSecondVehilcePara.m_collisionDetectedPosition[0].x > m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x) ?
					m_predSecondVehilcePara.m_collisionDetectedPosition[0].x : m_predSecondVehilcePara.m_collisionDetectedPosition[m_iFinalPointPosition].x;
				...
				if (m_fMinPosition <= m_fResult[m_cEquationSolutionNum] && m_fMaxPosition >= m_fResult[m_cEquationSolutionNum])
				{
					...
					/*double m_dVehiclePolynomialCoefficient[10] = { 0 };*/
					collisionDetected *m_firstCollisionDetected = new collisionDetected(/*LevelCoefficient,*/ m_predFisrtVehilcePara.m_collisionDetectedPosition);
					m_firstVehilceDistance = abs(m_firstCollisionDetected->GetLineLength(LevelCoefficient, m_predFisrtVehilcePara.m_dVehiclePolynomialCoefficient,
						m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x, m_fResult[m_cEquationSolutionNum]));

					collisionDetected *m_secondCollisionDetected = new collisionDetected(/*LevelCoefficient,*/m_predSecondVehilcePara.m_collisionDetectedPosition);
					m_secondVehilceDistance = abs(m_secondCollisionDetected->GetLineLength(LevelCoefficient, m_predSecondVehilcePara.m_dVehiclePolynomialCoefficient,
						m_predSecondVehilcePara.m_collisionDetectedPosition[0].x, m_fResult[m_cEquationSolutionNum]));


					//0+a+bx+cx2
					double m_firstVehilceCoefficient[4] = { 0,m_firstVehilceDistance,-1.0*m_predFisrtVehilcePara.m_collisionDetectedPosition[0].speed,0.5*libsumo::Vehicle::getAccel(m_predFisrtVehilcePara.vehicleName) };
					double m_secondVehilceCoefficient[4] = { 0,m_secondVehilceDistance,-1.0*m_predSecondVehilcePara.m_collisionDetectedPosition[0].speed,0.5*libsumo::Vehicle::getAccel(m_predSecondVehilcePara.vehicleName) };
					...
					/*m_firstVehicleTime = abs(m_firstCollisionDetected->GetLineLength(LevelCoefficient, m_fFirstVehiclePolynomialCoefficient,
						m_predFisrtVehilcePara.m_collisionDetectedPosition[0].x, m_fResult[i])) / m_predFisrtVehilcePara.m_collisionDetectedPosition[0].speed;

					m_secondVehicleTime = abs(m_secondCollisionDetected->GetLineLength(LevelCoefficient, m_fSecondVehiclePolynomialCoefficient,
						m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x, m_fResult[i])) / m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].speed;*/
						//	}
					...
					else if (m_fTimeBias > 0)
					{
						...
						m_firstDerivationFlag = m_firstCollisionDetected->Derivation(LevelCoefficient,
							m_predFisrtVehilcePara.m_dVehiclePolynomialCoefficient, m_fResult[m_cEquationSolutionNum], m_firstCofficientDerivation);

						m_secondDerivationFlag = m_firstCollisionDetected->Derivation(LevelCoefficient,
							m_predSecondVehilcePara.m_dVehiclePolynomialCoefficient, m_fResult[m_cEquationSolutionNum], m_secondCofficientDerivation);
						...
						if (m_firstDerivationFlag == true && m_secondDerivationFlag == true)
						{
							...
							if (m_firstVehicleTime < m_secondVehicleTime)
							{
								...
								if (m_predSecondVehilcePara.m_collisionDetectedPosition[0].speed > m_dPresumeCofficient*m_predFisrtVehilcePara.m_collisionDetectedPosition[0].speed)
								{
									//m_mapTwoVehilceNameCollision.insert(pair<std::string, std::string>(vehicleName[i], vehicleName[j]));
									/*changeVehicleSpeed(m_predFisrtVehilcePara.vehicleName, m_predSecondVehilcePara.vehicleName);
									m_bFlag = true;
								...
								double m_fDistanceBias = m_predFisrtVehilcePara.m_collisionDetectedPosition[0].speed*sin(m_fTheta)*m_fTimeBias;
								if (m_fDistanceBias >= (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].length + m_predSecondVehilcePara.m_collisionDetectedPosition[0].length) / 2)
								{
									std::vector<predVehilcePara>::iterator m_vecVehiclePar;
									for (m_vecVehiclePar = m_setPreVehiclePara.begin(); m_vecVehiclePar != m_setPreVehiclePara.end();)
									{
										if (m_vecVehiclePar->vehicleName == m_predFisrtVehilcePara.vehicleName || m_vecVehiclePar->vehicleName == m_predSecondVehilcePara.vehicleName)
										{
											libsumo::Vehicle::setMaxSpeed(m_vecVehiclePar->vehicleName, m_vecVehiclePar->vehicleProtectedSpeed);
											m_vecVehiclePar = m_setPreVehiclePara.erase(m_vecVehiclePar);
										}
										else
										{
											m_vecVehiclePar++;
							...
							{
								double m_dPresumeCofficient = 1.5;
								if (m_predFisrtVehilcePara.m_collisionDetectedPosition[0].speed > m_dPresumeCofficient*m_predSecondVehilcePara.m_collisionDetectedPosition[0].speed)
								{
									////m_mapTwoVehilceNameCollision.insert(pair<std::string, std::string>(vehicleName[i], vehicleName[j]));
									//changeVehicleSpeed(m_predFisrtVehilcePara.vehicleName, m_predSecondVehilcePara.vehicleName);
									//continue;
								...
								double m_fDistanceBias = m_predSecondVehilcePara.m_collisionDetectedPosition[0].speed*sin(m_fTheta)*m_fTimeBias;
								if (m_fDistanceBias >= (m_predSecondVehilcePara.m_collisionDetectedPosition[0].length + m_predFisrtVehilcePara.m_collisionDetectedPosition[0].length) / 2)
								{
									std::vector<predVehilcePara>::iterator m_vecVehiclePar;
									for (m_vecVehiclePar = m_setPreVehiclePara.begin(); m_vecVehiclePar != m_setPreVehiclePara.end();)
									{
...
MSNet::simulationStep() {
	...
	std::vector<predVehilcePara> m_vecPreVehilcePara;

	unsigned char m_uCLevelCoefficient = 4;
	int m_uPrePositionPointNum = 10;
	for (iter = info.begin(); iter != info.end(); iter++)
	{

		predVehilcePara m_predVehilcePara;
		...
		{
			collisionDetectedPosition m_collisionDetectedPosition;
			
			m_collisionDetectedPosition.x = trace[i].x;
			m_collisionDetectedPosition.y = trace[i].y;
			m_collisionDetectedPosition.z = trace[i].z;
			m_collisionDetectedPosition.speed = libsumo::Vehicle::getSpeed(*iter);
			m_collisionDetectedPosition.angle = libsumo::Vehicle::getAngle(*iter);
			m_collisionDetectedPosition.length =libsumo::Vehicle::getLength(*iter);
			m_collisionDetectedPosition.width = libsumo::Vehicle::getWidth(*iter);
			m_predVehilcePara.m_collisionDetectedPosition.push_back(m_collisionDetectedPosition);
			
		}
		
		MSVehicle* veh = libsumo::Vehicle::getVehicle(*iter);
		m_predVehilcePara.m_backPosition = veh->getBackPosition();
		veh = NULL;
		delete veh;
		
		if (m_predVehilcePara.m_collisionDetectedPosition[0].speed <= 0.5)
		{
			if ((int)(m_predVehilcePara.m_collisionDetectedPosition[0].angle - 90) % 180 < 10)
			{
				//double m_dStartY = m_predVehilcePara.m_collisionDetectedPosition[0].y;
				//double m_dBackY = m_predVehilcePara.m_backPosition.y();
				int m_iPara = (m_predVehilcePara.m_collisionDetectedPosition[0].x < m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].x) ? 1 : -1;
				for (int num = 0; num < m_uPrePositionPointNum; num++)
				{
					double a = (num - m_uPrePositionPointNum / 2);
					double b = m_predVehilcePara.m_collisionDetectedPosition[0].length *0.5;
					m_predVehilcePara.m_collisionDetectedPosition[num].y = m_predVehilcePara.m_collisionDetectedPosition[num].y +
						a * b*m_iPara;

				}
			
			}
			else if ((int)(m_predVehilcePara.m_collisionDetectedPosition[0].angle) % 180 < 10)
			{
				//double m_dStartX = m_predVehilcePara.m_collisionDetectedPosition[0].x;
				//double m_dBackX = m_predVehilcePara.m_backPosition.x();
				/*int m_iStart = (m_predVehilcePara.m_collisionDetectedPosition[0].y < m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].y) ? 0 : m_uPrePositionPointNum - 1;
				int m_iEnd = (m_predVehilcePara.m_collisionDetectedPosition[0].y > m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].y) ? 0 : m_uPrePositionPointNum - 1;*/
				int m_iPara = (m_predVehilcePara.m_collisionDetectedPosition[0].y < m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].y) ? 1 : -1;
				for (int num = 0; num < m_uPrePositionPointNum; num++)
				{
					double a = (num - m_uPrePositionPointNum / 2);
					double b = m_predVehilcePara.m_collisionDetectedPosition[0].length *0.5;
					m_predVehilcePara.m_collisionDetectedPosition[num].x = m_predVehilcePara.m_collisionDetectedPosition[num].x +
						a*b*m_iPara;
				}
			...
			{
				double m_dBiasY= m_predVehilcePara.m_collisionDetectedPosition[0].y- m_predVehilcePara.m_backPosition.y();
				double m_dBiasX= m_predVehilcePara.m_collisionDetectedPosition[0].x - m_predVehilcePara.m_backPosition.x();
				double m_dPara = m_dBiasY/m_dBiasX;
				/*int m_iStart = (m_predVehilcePara.m_collisionDetectedPosition[0].x < m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].x) ? 0 : m_uPrePositionPointNum - 1;
				int m_iEnd = (m_predVehilcePara.m_collisionDetectedPosition[0].x > m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].x) ? 0 : m_uPrePositionPointNum - 1;*/
				double  currentPositionX = m_predVehilcePara.m_collisionDetectedPosition[0].x;
				double currentPositionY = m_predVehilcePara.m_collisionDetectedPosition[0].y;
				int m_iPara = (m_predVehilcePara.m_collisionDetectedPosition[0].x < m_predVehilcePara.m_collisionDetectedPosition[m_uPrePositionPointNum - 1].x) ? 1 : -1;
				for (int num = 0; num < m_uPrePositionPointNum; num++)
				{
					double a = (num - m_uPrePositionPointNum / 2);
					double b = m_predVehilcePara.m_collisionDetectedPosition[0].length *0.5;
					m_predVehilcePara.m_collisionDetectedPosition[num].x = m_predVehilcePara.m_collisionDetectedPosition[num].x + a * b*m_iPara;
					m_predVehilcePara.m_collisionDetectedPosition[num].y = m_dPara * (m_predVehilcePara.m_collisionDetectedPosition[num].x - currentPositionX) + currentPositionY;
				}
			}
		}
		collisionDetected *m_collisionDetected = new collisionDetected(m_predVehilcePara.m_collisionDetectedPosition);
		m_predVehilcePara.m_bTraceIsJudgePositionXChange = m_collisionDetected->IsTwoCoordinateConsistent(m_predVehilcePara.m_collisionDetectedPosition, "row");
		m_predVehilcePara.m_bTraceIsJudgePositionYChange = m_collisionDetected->IsTwoCoordinateConsistent(m_predVehilcePara.m_collisionDetectedPosition, "col");
		int number = m_predVehilcePara.m_collisionDetectedPosition.size();
		m_collisionDetected->AugmentedMatrixAssignment((int)(number), m_uCLevelCoefficient, m_predVehilcePara.m_dVehiclePolynomialCoefficient);

		m_collisionDetected = NULL;
		delete m_collisionDetected;
		m_predVehilcePara.vehicleName = *iter;
		m_predVehilcePara.vehicleProtectedSpeed = libsumo::Vehicle::getSpeed(*iter);
		m_vecPreVehilcePara.push_back(m_predVehilcePara);
		trace.clear();
	}
	...
	{
		bool m_bFlag = false;
		for (unsigned int cols = 0; cols < m_setPreVehiclePara.size(); cols++)
		{
			for (unsigned int rows = cols + 1; rows < m_setPreVehiclePara.size(); rows++)
			{
				if (m_setPreVehiclePara[cols].vehicleName == mapSerach->first&&m_setPreVehiclePara[rows].vehicleName == mapSerach->second)
				{
					m_bFlag = true;
					break;
				}
				else if (m_setPreVehiclePara[rows].vehicleName == mapSerach->first&&m_setPreVehiclePara[cols].vehicleName == mapSerach->second)
				{
					m_bFlag = true;
					break;
				}
	...
	std::vector<predVehilcePara>::iterator  m_iterator;
	if (m_setPreVehiclePara.size() > 1e+3)
	{
		m_setPreVehiclePara.clear();
		return;
	}

	if (!m_setPreVehiclePara.empty())
	{
		for (m_iterator = m_setPreVehiclePara.begin(); m_iterator != m_setPreVehiclePara.end(); )
		{
			bool m_bFlag = false;
			for (iter = info.begin(); iter != info.end(); iter++)
			{
			...
			if (m_bFlag == false)
			{
				libsumo::Vehicle::setMaxSpeed(m_iterator->vehicleName, m_iterator->vehicleProtectedSpeed);
				m_iterator = m_setPreVehiclePara.erase(m_iterator);
	...
	if (m_uCLevelCoefficient <= 1 )
	{
		return;
	}
	for (unsigned int cols = 0; cols < m_vecPreVehilcePara.size()-1; cols++)
	{
		bool m_bIsRecoverSpeed = true;
		for (unsigned int rows = cols + 1; rows < m_vecPreVehilcePara.size(); rows++)
		{
			int number = m_uPrePositionPointNum;
			if (number <= 0)
			{
				continue;
			}
			bool m_bFlag = PredictCollision(m_vecPreVehilcePara[cols], m_vecPreVehilcePara[rows], number,m_uCLevelCoefficient);
			if (m_bFlag == true)
			{
				changeVehicleSpeed(m_vecPreVehilcePara[cols].vehicleName, m_vecPreVehilcePara[rows].vehicleName);
				m_bIsRecoverSpeed = false;
				//m_setPreVehiclePara
			}

		}
		if (m_bIsRecoverSpeed)
		{
			for (m_iterator = m_setPreVehiclePara.begin(); m_iterator != m_setPreVehiclePara.end(); )
			{
				if (m_iterator->vehicleName == m_vecPreVehilcePara[cols].vehicleName)
				{
					libsumo::Vehicle::setMaxSpeed(m_iterator->vehicleName, m_iterator->vehicleProtectedSpeed);
					m_iterator = m_setPreVehiclePara.erase(m_iterator);
				}
