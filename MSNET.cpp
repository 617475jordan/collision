...
//#include<algorithm>
	...
	for (iter = info.begin(); iter != info.end(); iter++)
	{
		...
		if (m_predVehilcePara.m_collisionDetectedPosition[0].speed <= 0.5)
		{
			if ((int)(m_predVehilcePara.m_collisionDetectedPosition[0].angle - 90) % 180 < 10)
			{
				...
				for (int num = 0; num < m_predVehilcePara.m_collisionDetectedPosition.size(); num++)
				{
					m_predVehilcePara.m_collisionDetectedPosition[num].y = m_predVehilcePara.m_collisionDetectedPosition[num].y +
						0.25*m_predVehilcePara.m_collisionDetectedPosition[num].length*(num - m_predVehilcePara.m_collisionDetectedPosition.size() / 2);

				}
				if (m_dBackY > m_dStartY&&m_predVehilcePara.m_collisionDetectedPosition.size()>1)
				{
					for (unsigned int num = 0; num < m_predVehilcePara.m_collisionDetectedPosition.size() / 2; num++)
					{
						collisionDetectedPosition n_collisionDetectedPosition = m_predVehilcePara.m_collisionDetectedPosition[num];
						m_predVehilcePara.m_collisionDetectedPosition[num] = m_predVehilcePara.m_collisionDetectedPosition[m_predVehilcePara.m_collisionDetectedPosition.size() - num - 1];
						m_predVehilcePara.m_collisionDetectedPosition[m_predVehilcePara.m_collisionDetectedPosition.size() - num - 1] = n_collisionDetectedPosition;
					}
				}
			}
			else if ((int)(m_predVehilcePara.m_collisionDetectedPosition[0].angle) % 180 < 10)
			{
				...
				if (m_dBackX > m_dStartX&&m_predVehilcePara.m_collisionDetectedPosition.size() > 1)
				{
					for (unsigned int num = 0; num < m_predVehilcePara.m_collisionDetectedPosition.size() / 2; num++)
					{
						collisionDetectedPosition n_collisionDetectedPosition = m_predVehilcePara.m_collisionDetectedPosition[num];
	...
	for (unsigned int cols = 0; cols < m_setPreVehiclePara.size(); cols++)
	{
		...
		int m_uIFirstVehiclePositionSize = m_setPreVehiclePara[cols].m_collisionDetectedPosition.size() - 1;
		...
		{
			int  m_uISecondVehiclePositionSize = m_setPreVehiclePara[rows].m_collisionDetectedPosition.size() - 1;
			if (m_uISecondVehiclePositionSize < 0)
			{
				continue;
			}

			double m_fMaxVehicleDistance = (m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].length > m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].length) 
				? m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].length : m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].length;
			m_fMaxVehicleDistance = m_fMaxVehicleDistance * 2;

			if (m_setPreVehiclePara[cols].m_bTraceIsJudgePositionYChange == false && m_setPreVehiclePara[rows].m_bTraceIsJudgePositionYChange == false
				&& abs(m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].x - m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x) <= m_fMaxVehicleDistance)//??????§Ò????????????????????????
			{
				double m_initialPositionY = m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].y - m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].y;
				double m_finalPositionY = m_setPreVehiclePara[cols].m_collisionDetectedPosition[m_uIFirstVehiclePositionSize].y -
					m_setPreVehiclePara[rows].m_collisionDetectedPosition[m_uISecondVehiclePositionSize].y;
				...
				if (m_initialPositionY*m_finalPositionY <= 0)
				{
					...
					changeVehicleSpeed(m_setPreVehiclePara[cols].vehicleName, m_setPreVehiclePara[rows].vehicleName);
			...
			if (m_setPreVehiclePara[cols].m_bTraceIsJudgePositionXChange == false && m_setPreVehiclePara[rows].m_bTraceIsJudgePositionXChange == false
				&& abs(m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].y - m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].y) <= m_fMaxVehicleDistance)//??????§Ò????????????????????????
			{
				double m_initialPositionX = m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].x - m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x;
				double m_finalPositionX = m_setPreVehiclePara[cols].m_collisionDetectedPosition[m_uIFirstVehiclePositionSize].x
					- m_setPreVehiclePara[rows].m_collisionDetectedPosition[m_uISecondVehiclePositionSize].x;
			...
			if (m_setPreVehiclePara[cols].m_bTraceIsJudgePositionYChange == false && m_setPreVehiclePara[rows].m_bTraceIsJudgePositionYChange == true)//consdier that position which has been sorted,where the min position may be the first position
			{
				if (m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].x >= m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x
					&&m_setPreVehiclePara[cols].m_collisionDetectedPosition[m_uIFirstVehiclePositionSize].x <= m_setPreVehiclePara[rows].m_collisionDetectedPosition[m_uISecondVehiclePositionSize].x
				...
				else
				{
					...
					{
						if (m_vecVehiclePar->vehicleName == m_setPreVehiclePara[rows].vehicleName || m_vecVehiclePar->vehicleName == m_setPreVehiclePara[cols].vehicleName)
						{
			...
			else if (m_setPreVehiclePara[cols].m_bTraceIsJudgePositionYChange == true && m_setPreVehiclePara[rows].m_bTraceIsJudgePositionYChange == false)
			{
				if (m_setPreVehiclePara[cols].m_collisionDetectedPosition[0].x < m_setPreVehiclePara[rows].m_collisionDetectedPosition[0].x
					&&m_setPreVehiclePara[cols].m_collisionDetectedPosition[m_uIFirstVehiclePositionSize].x > m_setPreVehiclePara[rows].m_collisionDetectedPosition[m_uISecondVehiclePositionSize].x
					/*&&abs(m_vecVehcilePrePositon[i][0].y - m_vecVehcilePrePositon[j][0].x) <= (m_vecVehcilePrePositon[i][0].width + m_vecVehcilePrePositon[j][0].width) / 2.0*/)
			...
			else if (m_setPreVehiclePara[cols].m_bTraceIsJudgePositionXChange == true && m_setPreVehiclePara[cols].m_bTraceIsJudgePositionYChange == true &&
				m_setPreVehiclePara[rows].m_bTraceIsJudgePositionXChange == true && m_setPreVehiclePara[rows].m_bTraceIsJudgePositionYChange == true)
			{
