...
bool MSNet::changeVehicleSpeed(std::string &firstVehicleName, std::string &secondVehcileName)
{
	...
	std::cout <<"Start"<<" "<< "vehicleName:" << firstVehicleName << " " << m_dFirstVehicleSpeed <<
		"vehicleName:" << secondVehcileName << " " << m_dSecondVehicleSpeed << std::endl;
	...
	std::cout << "End" << " " << "vehicleName:" << firstVehicleName << " " << libsumo::Vehicle::getSpeed(firstVehicleName) <<
		"vehicleName:" << secondVehcileName << " " << libsumo::Vehicle::getSpeed(secondVehcileName) << std::endl;
	return true;
}
...
MSNet::simulationStep() {
	...
	Position position(174.72, 1576.29, 10);
	...
	for (int i = 0; i < vehicleName.size(); i++)
	{
		...
		for (int j = i + 1; j < vehicleName.size(); j++)
		{
			...
			if (m_firstTraceIsJudgePositionYChange == false && m_secondTraceIsJudgePositionYChange == false)//??????§Ò????????????????????????
			{
			...
			if (m_firstTraceIsJudgePositionXChange == false && m_firstTraceIsJudgePositionXChange == false)//??????§Ò????????????????????????
			{
			...
			{
				std::vector<collisionDetectedPosition> m_cmpVehiclePosition(m_uIFirstVehiclePositionSize);
