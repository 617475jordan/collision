...
collisionDetected::collisionDetected(std::vector<Position> vehcilePosition)
{
...
#pragma  omp parallel for reduction(+:m_fSum)
	...
	{
		m_fSum += pow(m_vehiclePosition[i].x, cols);
	}
	return m_fSum;
}
...
#pragma  omp parallel for  reduction(+:m_fSum)
	for (int i = 0; i < n; i++)
	{
		m_fSum += m_vehiclePosition[i].y * pow(m_vehiclePosition[i].x, k);
	}
	return m_fSum;
}
...
#pragma  omp parallel for reduction(+:m_quadratureSum)
		...
		{
			m_quadratureSum += 1.0 / (i + 1)*coefficient[i + 1] * (pow(finalValue, i + 1) - pow(initialtValue, i + 1));
		}
	}
...
#pragma omp section
					{
						m_startPositionYValue += coefficient[j] * pow(i, j);
					}
#pragma omp section
					{
						m_finalPositionYValue += coefficient[j] * pow(i + m_fStepPrecision[m_uCount], j);
					}
				}
...
bool collisionDetected::IsTwoCoordinateConsistent(std::vector<Position> m_currentPositon, std::string m_direction)
{
	bool m_bFlag = false;
	for (unsigned int i = 0; i < m_currentPositon.size() - 1 && m_currentPositon.size()>1; i++)
	{
		double m_currentPositionValue;
		double m_followPositionValue;
		if (m_direction == "col")
		{
...
#pragma  omp parallel for
		...
		{
			m_derivationSum = m_derivationSum + coefficient[i] * (i - 1)*pow(currentPositionXvalue, i - 1);
	...
	else if (number == 4)
	{
		...
		else
		{
			...
			if (delta < 0 && A>0)
			{
				...
				double theta = acos(T);
				result[0] = (-b - 2 * sqrt(A)*cos(theta / 3.0)) / (3 * a);
				result[1] = (-b + sqrt(A)*(cos(theta / 3.0) + sqrt(3)*sin(theta / 3.0))) / (3 * a);
				result[2] = (-b + sqrt(A)*(cos(theta / 3.0) - sqrt(3)*sin(theta / 3.0))) / (3 * a);
			...
			{
				double y1 = A * b + 3 * a * (-B + sqrt(delta))*0.5;
				double y2 = A * b + 3 * a * (-B - sqrt(delta))*0.5;
